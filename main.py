...
@app.get("/masalah/disposisi_1")
def app_get_masalah_disposisi_1_without(db: Session = Depends(get_db),
current_user: user_schema.User = Depends(get_current_admin)):
    response = get_masalah_by_disposisi_1(db=db, id_disposisi_1=current_user.id)
    try: return {"status": response[0], "message": response[1], "data": response[2]}
    except Exception as e: return {"status": False, "message": "Error: " + str(e), "data": []}
    else: del response
@app.get("/masalah/disposisi_2")
def app_get_masalah_disposisi_2_without(db: Session = Depends(get_db),
current_user: user_schema.User = Depends(get_current_sub_admin)):
    response = get_masalah_by_disposisi_2(db=db, id_disposisi_2=current_user.id)
    try: return {"status": response[0], "message": response[1], "data": response[2]}
    except Exception as e: return {"status": False, "message": "Error: " + str(e), "data": []}
    else: del response
@app.get("/masalah/disposisi_3")
def app_get_masalah_disposisi_3_without(db: Session = Depends(get_db),
current_user: user_schema.User = Depends(get_current_operator)):
    response = get_masalah_by_disposisi_3(db=db, id_disposisi_3=current_user.id)
    try: return {"status": response[0], "message": response[1], "data": response[2]}
    except Exception as e: return {"status": False, "message": "Error: " + str(e), "data": []}
    else: del response
...
def app_get_masalah_status_all(db: Session = Depends(get_db),
current_user: user_schema.User = Depends(get_current_user)):
    response = get_masalah_by_status(db=db, status="1", user_type="pelapor", id_user=current_user.id)
    try: return {"status": response[0], "message": response[1], "data": response[2]}
    except Exception as e: return {"status": False, "message": "Error: " + str(e), "data": []}
    else: del response
@app.post("/masalah")
